...
- Center coordinates: (0, 0.5) for C1, (0, -1.5) for C2
...
### Main Menu Options
1. **Create Datasets**: Generate RND, CTR, and EDGE datasets
2. **Display Current Dataset**: Visualize the three sampling methods
3. **Train on ALL Datasets**: Train selected architectures on all data
4. **Single Point Test**: Classify individual points with all trained models
5. **Cross-Dataset Comparison**: Test models trained on one dataset against all others
6. **List FP/FN Points**: Analyze misclassifications
9. **Cleanup**: Clear datasets and trim results file
